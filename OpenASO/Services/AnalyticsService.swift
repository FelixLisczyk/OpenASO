...
final class AppSettingsStore {
    ...
    init(defaults: UserDefaults = .openASOShared) {
        ...
        let storedPopularityContextAppStoreID = defaults.string(forKey: DefaultsKey.popularityContextAppStoreID)
        let storedPopularityContextStorefrontCode = defaults.string(forKey: DefaultsKey.popularityContextStorefrontCode)
        ...
        self.popularityContextStorefrontCode = Self.normalizedStorefrontCode(storedPopularityContextStorefrontCode)
        self.isAutomaticRefreshEnabled = storedIsEnabled ?? Self.defaultIsAutomaticRefreshEnabled
        self.refreshTimeMinutes = Self.normalized(minutes: storedMinutes ?? Self.defaultRefreshTimeMinutes)
        self.lastRefreshTriggeredAt = defaults.object(forKey: DefaultsKey.lastRefreshTriggeredAt) as? Date
        self.lastRatingsReviewsRefreshAt = defaults.object(forKey: DefaultsKey.lastRatingsReviewsRefreshAt) as? Date
    ...
    func nextRefreshCheckDate(after date: Date, calendar: Calendar = .current) -> Date {
        guard isAutomaticRefreshEnabled else {
            return calendar.date(byAdding: .day, value: 1, to: date) ?? date.addingTimeInterval(60 * 60 * 24)
        }
        ...
        let tomorrow = calendar.date(byAdding: .day, value: 1, to: date) ?? date.addingTimeInterval(60 * 60 * 24)
...
extension AnalyticsEvent {
    ...
    static func trackedAppAdded(platform: AppPlatform, source: String) -> AnalyticsEvent {
        AnalyticsEvent(name: "tracked_app_added", properties: ["platform": platform.rawValue, "source": source])
    }

    static func trackedAppRemoved(keywordCount: Int) -> AnalyticsEvent {
        AnalyticsEvent(name: "tracked_app_removed", properties: ["keyword_count_bucket": AnalyticsCountBucket(keywordCount).rawValue])
    }
    ...
    static func keywordDeleted(deleteCount: Int) -> AnalyticsEvent {
        AnalyticsEvent(name: "keyword_deleted", properties: ["delete_count_bucket": AnalyticsCountBucket(deleteCount).rawValue])
    }

    static func keywordRefreshStarted(trigger: String, trackCount: Int) -> AnalyticsEvent {
        AnalyticsEvent(
            name: "keyword_refresh_started",
            properties: ["trigger": trigger, "track_count_bucket": AnalyticsCountBucket(trackCount).rawValue]
        )
    }

    static func keywordRefreshCompleted(trigger: String, trackCount: Int, failureCount: Int) -> AnalyticsEvent {
        AnalyticsEvent(
    ...
    static func ratingsRefreshStarted(trigger: String, storefrontScope: String) -> AnalyticsEvent {
        AnalyticsEvent(name: "ratings_refresh_started", properties: ["trigger": trigger, "storefront_scope": storefrontScope])
    }

    static func ratingsRefreshCompleted(trigger: String, storefrontScope: String, failureCount: Int) -> AnalyticsEvent {
        AnalyticsEvent(
